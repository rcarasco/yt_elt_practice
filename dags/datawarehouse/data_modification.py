...
def insert_rows(cur, conn, schema, row):
    try:
        ...
        conn.commit()
        logger.info(f"Upserted row with Video_ID: {row[video_id_key]}")

    except Exception as e:
        conn.rollback() # Important: Rollback the transaction on failure
        logger.error(f"Error upserting row with Video_ID: {row.get(video_id_key, 'unknown')}")
        raise e
